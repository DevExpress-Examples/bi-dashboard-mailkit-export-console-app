...
Namespace ConsoleMailExport
    Friend Class Program
        ...
        Private Shared Function GetTeamsInformation() As List(Of TeamInformation)
            Dim teams As List(Of TeamInformation) = New List(Of TeamInformation)()
            teams.Add(New TeamInformation() With {.TeamName = "Management Team", .TeamMail = "management@somewhere.com", .TeamRegions = New List(Of String) From {"Eastern", "Northern", "Southern", "Western"}})
            teams.Add(New TeamInformation() With {.TeamName = "Eastern Team", .TeamMail = "eastern@somewhere.com", .TeamRegions = New List(Of String) From {"Eastern"}})
            teams.Add(New TeamInformation() With {.TeamName = "Northern Team", .TeamMail = "northern@somewhere.com", .TeamRegions = New List(Of String) From {"Northern"}})
            teams.Add(New TeamInformation() With {.TeamName = "Southern Team", .TeamMail = "southern@somewhere.com", .TeamRegions = New List(Of String) From {"Southern"}})
            teams.Add(New TeamInformation() With {.TeamName = "Western Team", .TeamMail = "western@somewhere.com", .TeamRegions = New List(Of String) From {"Western"}})
            Return teams
        End Function
        ...
        Private Shared Function CreateMimeMessage(ByVal team As TeamInformation) As MimeMessage
            Try
                ...
                Dim regionFilterState As DashboardState = New DashboardState()
                Dim itemState As DashboardItemState = New DashboardItemState("listBoxDashboardItem1")
